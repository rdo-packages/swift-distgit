...
# Required by sphinx-1.2.3 on F23. Ironically, it immediately quits with
#   fatal: Not a git repository
# But without git installed, Sphinx blows up with
#   error: [Errno 2] No such file or directory
# I wish I could make up something like that.
...
# samples (used to be installed automagially before 2.7.0)
install -d -m 755 %{buildroot}%{_defaultdocdir}/%{name}
for d in etc/*-sample; do
  install -p -m 0644 $d %{buildroot}%{_defaultdocdir}/%{name}
done

# this is generated by a mysterious build process; used to be README.rst
install -p -m 0644 README.md %{buildroot}%{_defaultdocdir}/%{name}
...
%changelog
* Mon May 30 2016 Pete Zaitcev <zaitcev@redhat.com> 2.7.0-0.2
- Move all the code into a common subpackage; enough with ImportError
- Do the same for etc/*-sample; otherwise they get scattered with package names
- Drop the now-unnecessary Requires: openstack-swift-container from the proxy
